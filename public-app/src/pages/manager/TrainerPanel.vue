...
export default {
  ...
  beforeMount () {
    ...
    if(this.facility && this.facility.name){
      this.$store.dispatch('users/fetchFacilityTrainers', { facility_id: this.facility.name });
    }
  },
  computed:{
    facility(){
...
<template>
  <div>
    <users-grid :noDispatch="true" :compact="true" :noPadding="true" class="grid"></users-grid>
  </div>
</template>
